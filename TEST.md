...
+ Using the MSFragger results as input
```
nextflow \
...
+ Using the RefMod resultas as imput
```
nextflow \
    -log "/tmp/nextflow/log/nf-ptm-compass.log" \
    run main.nf   \
        -profile singularity \
        --sch_files "samples/heteroplasmic_heart/inputs/refmod/*.tsv" \
        --exp_table "samples/heteroplasmic_heart/inputs/experimental_table.tsv" \
        --database "samples/heteroplasmic_heart/inputs/database.fasta" \
        --decoy_prefix "DECOY_"\
        --params_file "samples/heteroplasmic_heart/inputs/params_nf.ini" \
...
nextflow \
    ...
    run main.nf   \
        ...
        -resume
```
+ Using the REFMOD results as input
```
nextflow \
    -log "/tmp/nextflow/log/nf-ptm-compass.log" \
    run main.nf   \
